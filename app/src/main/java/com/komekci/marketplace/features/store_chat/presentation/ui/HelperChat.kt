...
import com.komekci.marketplace.features.chat.presentation.ui.ChatsItem
...
@Composable
fun HelperChat(
    chatsViewModel: StoreChatsViewModel = hiltViewModel(),
    chatViewModel: StoreOnChatViewModel = hiltViewModel(),
    navHostController: NavHostController,
    onChatClick: (ChatsModel) -> Unit = {}
) {
    val strings = LocalStrings.current

    val state = chatsViewModel.komekchiListState.collectAsState()

    LaunchedEffect(true) {
        chatsViewModel.initHelperList()
    }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        if (state.value.loading) {
            ...
            AppError(modifier = Modifier.fillMaxSize()) {
                chatsViewModel.getHelperList()
            }
        } else if (state.value.list.isNullOrEmpty()) {
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                ...
                state.value.list?.let { list ->
                    items(list) { item ->
                        ChatsItem(item = item.toChatModel()) {
                            chatsViewModel.createHelperChat(item.id?:0, onSuccess = { roomId->
                                val route = Routes.OnChat.replace("{roomId}", roomId)
                                onChatClick(item.toChatModel().copy(
                                    roomId = roomId
                                ))
